...
def run_dns_server():
    ...
    while True:
        try:
            ...
            request = dns.message.from_wire(data) #this is one
            # Create a response message using the `dns.message.make_response` method
            response = dns.message.make_response(request) #this is one

            # Get the question from the request
            question = request.question[0] #this is one
            ...
            if qname in dns_records and qtype in dns_records[qname]:
                ...
                if qtype == dns.rdatatype.MX: #this is one
                    ...
                    if isinstance(answer_data, str):
                        ...
                        for current, rdlen, data in answer_data:
                            rdata_list = [dns.rdata.from_wire(dns.rdataclass.IN, qtype, current, rdlen, data) for data in answer_data]
            ...
            server_socket.sendto(response.to_wire(), addr) #this is one 
